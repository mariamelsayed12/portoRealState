...
const SellPage = () => {
  ...
  const onSubmit = async (data: SellFormData) => {
    ...
    try {
      ...
      toast.success(t("sell.successMessage"));
      reset();
    } catch (error) {
      console.error(error);
      toast.error(t("sell.errorMessage"));
    } finally {
  ...
  const getValidationError = (msg?: string) => {
    ...
    switch (msg.toLowerCase()) {
      case "full name is required":
        return t("sell.validation.fullNameRequired");
      case "full name must be at least 3 characters":
        return t("sell.validation.fullNameMin");
      case "phone number is required":
        return t("sell.validation.phoneNumberRequired");
      case "phone number must be digits only":
        return t("sell.validation.phoneNumberDigits");
      case "phone number must be at least 8 digits":
        return t("sell.validation.phoneNumberMin");
      default:
        return msg;
    }
  };
  ...
  const getPhoneCodeLabel = (code: string) => {
    switch (code) {
      case "+20":
        return `${t("sell.phoneCodes.eg")} ${code}`;
      case "+971":
        return `${t("sell.phoneCodes.uae")} ${code}`;
      case "+966":
        return `${t("sell.phoneCodes.ksa")} ${code}`;
      case "+44":
        return `${t("sell.phoneCodes.uk")} ${code}`;
      case "+1":
        return `${t("sell.phoneCodes.us")} ${code}`;
      default:
        return code;
    }
  };
              ...
              <div className="flex flex-col gap-[16px]">
                ...
                <div className="flex flex-col gap-[8px]">
                  <label className="text-[16px] font-normal font-['Poppins'] text-[#141414]">
                    {t("sell.form.fullName")}<span className="text-[#1E8CAB] ml-[2px] rtl:mr-[2px]">*</span>
                  </label>
                  <Input
                    type="text"
                    placeholder={t("sell.form.fullNamePlaceholder")}
                    {...register("fullName")}
                ...
                <div className="flex flex-col gap-[8px] h-auto">
                  <label className="text-[16px] font-normal font-['Poppins'] text-[#141414]">
                    {t("sell.form.phoneNumber")}<span className="text-[#1E8CAB] ml-[2px] rtl:mr-[2px]">*</span>
                  </label>
                  <div className="flex gap-[8px] h-[48px]">
                    ...
                    <Input
                      type="tel"
                      dir="ltr"
                      placeholder={t("sell.form.phoneNumberPlaceholder")}
