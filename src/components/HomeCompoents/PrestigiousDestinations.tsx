    ...
    <section className="max-w-6xl mx-auto  py-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {destinations.map((dest, index) => (
          <div
            key={index}
            className="group relative aspect-[3/4] rounded-[24px] sm:rounded-[32px] overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 cursor-pointer"
          >
            {/* Background image */}
            <img
              src={defualtImage}
            ...
            <div className="absolute bottom-4 left-4 right-4 z-20 backdrop-blur-md bg-black/40 border border-white/10 rounded-2xl p-4 text-white flex justify-between items-center shadow-md">
              <div>
                <h3 className="text-lg font-bold tracking-wide">{dest.title}</h3>
                <p className="text-xs text-white/70 font-medium mt-0.5">{dest.developer}</p>
              </div>
              <div className="text-right">
                <span className="block text-[8px] uppercase tracking-widest text-white/50 font-bold">
                  Starts from
                </span>
                <span className="block text-sm font-extrabold text-primary mt-0.5">
                  {dest.price}
                </span>
              </div>
            </div>
